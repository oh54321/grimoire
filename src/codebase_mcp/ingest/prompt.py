...
def build_ingest_prompt(source: str, kind: str = "auto") -> str:
    ...
    return (
        ...
        "4. For each kept symbol: read_source(session, module, symbol) -> define(...) "
        "with the right kind/is_tool/searchable -> write real tests -> implement(...). "
        "Fix until the pytest gate passes (the only way code enters the library).\n"
